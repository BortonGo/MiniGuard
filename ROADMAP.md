...
**Этап 2 (File Monitor): продолжить от уже собранного и запущенного
`fanotify_init()`.** В начале следующей сессии нужно выполнить
`git pull --ff-only`, затем разобрать назначение fanotify group, оба аргумента
`fanotify_init()`, close-on-exec флаги, владение queue fd и ограничения прав.
После этого владелец самостоятельно добавляет один `fanotify_mark()` для
отдельного безопасного тестового каталога. Event loop пока не добавлять.
...
- [ ] Разобраны результат запуска, влияние обычного пользователя/`sudo`, возможные
  `EPERM`/`EINVAL` и ограничения конкретной версии ядра.
