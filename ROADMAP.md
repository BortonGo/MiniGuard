...
- `SIGINT` заблокирован через `sigprocmask()` и преобразован в читаемый
  file descriptor через `signalfd()`.
- `poll()` одновременно ожидает готовность fanotify queue fd и signal fd;
  результаты проверяются через `revents`.
- В Ubuntu подтверждена обработка двух последовательных `FAN_OPEN` событий и
